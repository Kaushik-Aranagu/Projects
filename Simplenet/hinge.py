...
def main():
  ...
  # Classifier has only one layer (net.module.classifier)
  # This is the list of parameters in the last layer of the network with only one layer in the classifier. Used when only the classifeir layers are to be trained
  ...
  # This is the list of parameters in the last two layers of the network. Used when the layers in the classifier are to be trained
  # (Uncomment for training only the classifier layers and the rest of the layers of the network frozen)
  ...
  for epoch in range(args.start_epoch, args.epochs):
    ...
    # Save checkpoint after every 40 epochs
    if epoch % 40 == 39:
        save_checkpoint({
          'epoch': epoch ,
          'arch': args.arch,
        ...
        }, False, args.save_path, 'hingeloss_2layer_frozen.ckpt'.format(epoch), time_stamp)


    # measure elapsed time
    epoch_time.update(time.time() - start_time)
    start_time = time.time()
    recorder.plot_curve( os.path.join(args.save_path, 'training_plot_hingeloss_{0}_1layer_finetuning.png'.format(args.manualSeed)) )
...
def accuracy(output, target, topk=(1,)):
  ...
  # We only take first 100 of the 130 nodes because only the first 100 nodes are the classifiers on the true classes(No internal node)
